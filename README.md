...
\quad
\begin{aligned}
&s_1=\texttt{dtile}\in[0,1] \text{(We define `dtile = \min(1, D / D_{\max})`, where \( D \) is Manhattan distance in tiles and \( D_{\max} \) is the arena’s max tile distance; higher means farther.)},\\
