# GA-Maze: A Genetic‑Algorithm Bot in a Dynamic Grid Arena (Unity)
...
### Maze Generation
- Cells form an \(m\times n\) grid. A depth‑first search (DFS) carves a **perfect maze** (single path between any two cells), then a second pass **adds loops** by knocking down extra walls with probability \(p_{loop}\).
...
- **Teleport:** preview a marker \(k\) cells ahead; confirm to blink and pay cost
  \[
    \text{Cost}_{\text{TP}}(k) = c_0 + c_1\,k,\quad k\in\mathbb{N}.
  \]
...
- **Linear policy**: each action \(a\) has weights \(\mathbf{w}^{(a)}\in\mathbb{R}^5\). The score is
  \[
  \text{Score}(a\mid\mathbf{s}) = \mathbf{w}^{(a)}\cdot\mathbf{s} = \sum_{j=1}^{5} w^{(a)}_j s_j,\quad a^* = \arg\max_a \text{Score}(a\mid\mathbf{s}).
  \]
